...
function CoAPServer(options, listener, socket) {
  if (!(this instanceof CoAPServer)) {
    return new CoAPServer(options, listener, socket)
  }
  ...
  //store to agentSocket
  this._agentSocket = LRU({
    max: options.cacheSize || (32768 * 1024),
    length: function(n) {
      return n.length
    },
...
CoAPServer.prototype.listen = function(done) {
  ...
  if (this._options.address == undefined) {
    ...
    this._address = this._options._address;
  }
  ...
  console.log("Listen ", this._port, this._address)
    
  //e dtls
  if(this._sock) {
    console.log("CRIANDO THIS SOCK")
    if(this._options.dtls && this._options.dtls == true){
      ...
      this._sock.on('data', msg => {
         /*
          var request = {
            raw:    msg,
            rsinfo: {address: this._sock.remoteAddress, port: this._sock.remotePort},
            server: that
          };        */
          //var request = {
            //raw:         msg,
          ...
          //}
          console.log("adicionando handler")  
        ...
        try {
              ...
              this._handle(msg, {address: this._sock.remoteAddress, port: this._sock.remotePort}, this._sock)
            } catch (ex) {
        ...
        this._sock.once('close', () => {
          this._sock._closeSocket()           
        //  if (done) done()
        });    
    }
    
    console.log("Already listening: ", this._address, this._port);
  ...
  if (this._options.dtls) {
    ...
    this._dtls_server = dtls.createServer(this._options.dtls, socket => {
      ...
      socket.on('data', msg => {
        ...
        this._handle(msg, {address: socket.remoteAddress, port: socket.remotePort}, socket)
    ...
    this._dtls_server.listen(this._port);
  }
  else {
    // Non- DTLS workflow...
...
CoAPServer.prototype.addRequestStore = function(req){  
  
  this._agentSocket.set(req._lastMessageId,req)
  //agetStore.push(req);
 // console.log("REQUESTS STORE", agetStore)
  
}

CoAPServer.prototype._handle = function(msg, rsinfo, sock_pass_in) {
  var packet = parse(msg);
  
  console.log("_handle** SERVER ", rsinfo);
    console.log("_handle** server", packet.payload.toString("utf8"));

    this._agentSocket.forEach(function (val, key, cache) {
          //console.log("cache", cache)            
      if(val._msgIdToReq[packet.messageId]){
        console.log("cache")
          val._handle(msg, rsinfo, sock_pass_in.address)             
          return;        
    }
  })
    

  //console.log("this._sock", sock_pass_in);
