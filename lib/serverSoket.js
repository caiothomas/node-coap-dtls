...
CoAPServer.prototype._sendReverseProxied = function(packet, rsinfo, callback) {
  var host = rsinfo.address,
    ...
    console.log("_sendReverseProxied", rsinfo)
    ...
    if(this._options.dtls && this._options.dtls == true){
      this._sock.on('data', msg => {
        ...
        try {
              ...
              this._handle(packet, {address: this._sock.remoteAddress, port: this._sock.remotePort}, this._sock)
            } catch (ex) {
...
CoAPServer.prototype._handle = function(packet, rsinfo, sock_pass_in) {
  ...
  console.log("_handle** server", packet);
