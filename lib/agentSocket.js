...
function Agent(opts, dtlsOpts, socket,  cb) {
...
Agent.prototype._init = function initSock(dtlsOpts, newCallback) {
  ...
  if (dtlsOpts && typeof(dtlsOpts) != 'function') {
    ...
    var bigSocket = dtlsClient.connect(dtlsOpts, socket => {
      socket.on('data', msg => {
        ...
        var addrs = that._sock.address();
        console.log("big socket ag")
        that._handle(msg, outSocket, addrs)
      });
     this._sock = socket;
    });
...
Agent.prototype._handle = function handle(msg, rsinfo, outSocket) {
  console.log("Agent Socket", msg.toString("utf8"))
  console.log("handle agent ", rsinfo);
  console.log("handle outSocket ", outSocket);
  ...
  if (packet.confirmable) {
    ...
    this._msgInFlight++;
    console.log("_handle() send 0 confim \n");
    this._sock.send(buf, 0, buf.length, rsinfo.port, rsinfo.address, ackSent);
  }
  console.log("fase handler 5\n")
  ...
  console.log("fase handler 7\n")
  
  if (packet.code == '0.00')
    return

    
  console.log("fase handler 8\n")
  ...
  if (req.url.observe && packet.code !== '4.04') {
   console.log("REQUEST OBSERVER REPONSE", response)
 ...
 //this.sock.removeListener('connection', callback);
...
Agent.prototype.request = function request(url, dtlsOpts) {
  ...
  req.sender.on('sending', function() {   
    console.log("send!!! agent", that._msgInFlight);
    that._msgInFlight++
  })
  ...
  if (url.observe) {
    ...
    console.log("close cleanUp!")   
    req.on('response', this._cleanUp.bind(this))
    //req.on('response', () => {
    //  req.sender.reset();
    //  that._cleanUp.bind(this);
    //});
  }
...
Agent.prototype.eventMsg = function eventMsg(msg, url){
   var packet, message, outSocket
         var that = this;
         ...
         outSocket = {
           port: url.port,
           address: url.hostname || url.host,
           family: that._opts.type === 'udp4' ? 'IPv4' : 'IPv6'
         };

         var addrs = that._sock.address;
         console.log("big socket ag", outSocket)
         that._handle(msg, outSocket, addrs)          
}

Agent.prototype.requestSocket = function request(url, dtlsOpts, server) {
  //this._init(dtlsOpts);
   console.log("AgentSocket ")
   console.log("Aqui remove removeAllListeners")   
   
   //this._sock.removeAllListeners('error');   
   //this._sock.removeAllListeners('data');
   //this._sock.removeAllListeners('close');
   console.log("count data", events.EventEmitter.listenerCount(this._sock, 'data'));
  //if(events.EventEmitter.listenerCount(this._sock, 'data') <= 2){
   //this._sock.pause();  
   //this._sock.removeAllListeners('data');
   //this._sock.resume();
 ...
 if(url.observe){
   ...
   this._sock.resume();  
   console.log("observe")
    this._sock.on('data', msg => {
         this.eventMsg(msg, url)     
   });   
 } else {
    this._sock.once('data', msg => {
         this.eventMsg(msg, url)         
   });   
 }  
///  }
